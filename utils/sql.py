...
class Connection:
    ...
    def get_playlist_info(self, idplaylist):
        ...
        # query = 'SELECT pl.name, pl.source, COUNT(s.id), pu.uuid, pl.url FROM "playlists" as pl, "songs_in_user_playlist" as s, "playlist_users" as pu '
        # query += 'where pl.id = ? and s.idPlaylist = ? and pu.idPlaylist = ? and pu.idUser = ? group by pl.name, pl.source, pu.uuid, pl.url'
    ...
    def insert_playlist(self, name, url, source):
        try:
            self.cursor.execute('INSERT INTO "playlists" (name, url, source) values (?, ?, ?)', [name, url, source])
            self.cursor.commit()
            return True
        except pyodbc.IntegrityError:
            return False
    ...
    def scrap_spotify_playlist(self, token, spotify_user, listaid, uuid, need_id=False):
        for cancion in YTMP3.get_sp_tracknames(token=token, user=spotify_user, listaid=listaid):
            try:
